...
GENERIC_STRING_VALUES = ["normal", "0", "ready", "job-end", "page", "printer", "option", "x" * 32]
...
def make_ppd(kind: str, case_index: int) -> str:
    if kind in FILTER_COVERAGE_PPDS:
        ...
        return _base_ppd(
            ...
            page_size=PAGE_SIZES[case_index % len(PAGE_SIZES)],
        )
    ...
    if kind == "pwg_resolution_coverage":
        dpi = COVERAGE_RESOLUTIONS[case_index % len(COVERAGE_RESOLUTIONS)]
        return _base_ppd(
...
def _coverage_options(case_index: int) -> str:
    color_model = ["Gray", "RGB", "CMYK", "Black"][case_index % 4]
    quality = ["Draft", "Normal", "High", "Photo"][(case_index // 2) % 4]
    media = ["Plain", "Glossy", "Transparency", "Envelope"][(case_index // 3) % 4]
    duplex = ["None", "DuplexNoTumble", "DuplexTumble"][(case_index // 5) % 3]
