...
from smt_fuzzer.multitarget_runner import load_profiles
from smt_fuzzer.ppd_templates import make_ppd


class PPDTemplateTests(unittest.TestCase):
    def test_plain_rastertopclx_template_has_no_b5_payload(self) -> None:
    ...
    def test_string_sweep_template_uses_non_format_values(self) -> None:
        ppd = make_ppd("rastertopclx_string_sweep", 5)
        self.assertIn("*cupsPCL EndJob", ppd)
        self.assertNotIn("%" + "s", ppd)
        self.assertNotIn("%" + "n", ppd)
    ...
    def test_parser_target_config_loads_all_profiles(self) -> None:
        ...
        self.assertEqual(profiles[0].id, "ppd_text_to_rastertopclx_discovery")
