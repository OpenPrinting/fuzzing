...
python3 -m parser_fuzzers.cli solve-event --event "$EVENT" --output "$RESULT" "${solve_args[@]}"
patched_path="$(python3 -m parser_fuzzers.cli patch-input --result "$RESULT" --output-dir "$OUT_DIR")"
...
from parser_fuzzers.solver import condition_holds, read_event_value
