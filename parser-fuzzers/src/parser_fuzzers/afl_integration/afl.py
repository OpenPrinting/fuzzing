...
from parser_fuzzers.experiment import ExperimentRow, load_experiment_rows
...
def build_afl_plan(
    ...
    *,
    ...
    if input_dir is None:
        input_path, smt_count = prepare_input_corpus(root_path, settings, target.id, row)
    else:
        ...
        if not input_path.exists():
            raise FileNotFoundError(f"AFL++ input directory does not exist: {input_path}")
    output_path = Path(output_dir) if output_dir is not None else root_path / settings.output_dir / target.id / row.id
    ...
    argv = [
        ...
        str(output_path),
        "-m",
        str(memory_mb if memory_mb is not None else settings.memory_mb),
        "-t",
        str(timeout_ms if timeout_ms is not None else settings.timeout_ms),
    ]
    if duration_sec is not None and duration_sec > 0:
        argv.extend(["-V", str(duration_sec)])
...
def run_afl_plan(plan: AFLPlan, *, execute: bool, require_instrumented: bool = True) -> int:
    if not execute:
    ...
    if not binary.exists():
        ...
        return 2
    if require_instrumented and not binary_looks_afl_instrumented(binary):
        print(f"target binary does not look AFL++ instrumented: {binary}")
        print("build it with afl-clang-fast/afl-clang-fast++ or pass --allow-non-instrumented explicitly")
        return 2
    env = os.environ.copy()
    env.update(plan.env)
...
def binary_looks_afl_instrumented(path: str | Path) -> bool:
    try:
        data = Path(path).read_bytes()
    except OSError:
        return False
    return b"__afl" in data or b"AFL++" in data or b"afl-compiler-rt" in data
